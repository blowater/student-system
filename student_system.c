...
course *create_cos(char id[][6],                    // 从已有数据导入一个课程结点
                    ...
                    char ioc[][20],
                    char iom[][20]);
course *tch_create_cos(course *fnode);                              //  手动创建教师开设课程结点
...
int tch_check_course(course *fnode, course *new_node)
{
    course *np;
    np = fnode;
    int i=0;
...
void std_select_cos(course *s_fcourse, course *t_fcourse)  //  学生选课
{
    ...
    np->selected++;
    int a=0;
    a = std_check_course(s_fcourse, np);
    if (a == 0)
    {
